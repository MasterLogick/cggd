...
void cg::renderer::dx12_renderer::initialize_device(ComPtr<IDXGIFactory4> &dxgi_factory) {
    ComPtr<IDXGIAdapter1> hardware_adater;
    dxgi_factory->EnumAdapters1(0, &hardware_adater);
#ifndef NDEBUG
    DXGI_ADAPTER_DESC desc{};
    hardware_adater->GetDesc(&desc);
    OutputDebugString(desc.Description);
    OutputDebugString(L"");
#endif
    THROW_IF_FAILED(D3D12CreateDevice(hardware_adater.Get(), D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(&device)));
}
...
void cg::renderer::dx12_renderer::create_root_signature(const D3D12_STATIC_SAMPLER_DESC *sampler_descriptors,
    ...
    rsDesc.Init_1_1(_countof(root_parameters), root_parameters, num_sampler_descriptors, sampler_descriptors, rsFlags);
    ComPtr<ID3DBlob> signature;
    ComPtr<ID3DBlob> error;

    HRESULT res = D3DX12SerializeVersionedRootSignature(&rsDesc, rsFeatureData.HighestVersion, &signature, &error);
    if (FAILED(res)) {
...
void cg::renderer::dx12_renderer::create_pso(const std::string &shader_name) {
    ComPtr<ID3DBlob> vertexShader = compile_shader(get_shader_path(shader_name), "VSMain", "vs_5_0");
    ComPtr<ID3DBlob> pixelShader = compile_shader(get_shader_path(shader_name), "PSMain", "ps_5_0");
    ...
    THROW_IF_FAILED(device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&pipeline_state)));
}
...
void cg::renderer::dx12_renderer::populate_command_list() {
    ...
    D3D12_RESOURCE_BARRIER beginBarriers[] = {
            CD3DX12_RESOURCE_BARRIER::Transition(render_targets[frame_index].Get(), D3D12_RESOURCE_STATE_PRESENT,
    ...
    for (int i = 0; i < model->get_index_buffers().size(); ++i) {
        ...
        command_list->DrawIndexedInstanced(static_cast<UINT>(model->get_index_buffers()[i]->get_number_of_elements()),
                                           1, 0, 0, 0);
    }
    D3D12_RESOURCE_BARRIER endBarriers[] = {
            CD3DX12_RESOURCE_BARRIER::Transition(render_targets[frame_index].Get(), D3D12_RESOURCE_STATE_RENDER_TARGET,
...
D3D12_CPU_DESCRIPTOR_HANDLE cg::renderer::descriptor_heap::get_cpu_descriptor_handle(UINT index) const {
    return CD3DX12_CPU_DESCRIPTOR_HANDLE(heap->GetCPUDescriptorHandleForHeapStart(), static_cast<INT>(index),
                                         descriptor_size);
}

D3D12_GPU_DESCRIPTOR_HANDLE cg::renderer::descriptor_heap::get_gpu_descriptor_handle(UINT index) const {
    return CD3DX12_GPU_DESCRIPTOR_HANDLE(heap->GetGPUDescriptorHandleForHeapStart(), static_cast<INT>(index),
                                         descriptor_size);
}
