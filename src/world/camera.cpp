...
const DirectX::XMMATRIX cg::world::camera::get_dxm_view_matrix() const {
    DirectX::FXMVECTOR eyePos{position.x, position.y, position.z};
    DirectX::FXMVECTOR eyeDir{get_direction().x, get_direction().y, get_direction().z};
    DirectX::FXMVECTOR up{get_up().x, get_up().y, get_up().z};
    return DirectX::XMMatrixLookToRH(eyePos, eyeDir, up);
}
...
const float4x4 cg::world::camera::get_projection_matrix() const {
    float f = 1.0f / std::tanf(angle_of_view / 2.0f);
    return float4x4{
